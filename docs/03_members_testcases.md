...
- Bulk row selection behavior across pagination
- Add Member flow
...
| MEM-021A | Select one member row using checkbox | Owner/Admin | Members list contains at least one record | 1. Open `/members` 2. Tick one row checkbox | Only that member becomes selected. Selection count and bulk action state update correctly. | High |
| MEM-021B | Select all rows on the current page | Owner/Admin | Members list contains multiple visible rows | 1. Open `/members` 2. Tick header checkbox | All visible rows on the current page become selected. Header checkbox reflects the selected state correctly. | High |
| MEM-021C | Retain selected rows when moving between pagination pages | Owner/Admin | Members list contains more than one page | 1. Select one or more rows on page 1 2. Move to page 2 3. Return to page 1 | Earlier selections are retained across pagination during the same working session. | High |
| MEM-021D | Add second-page records into an existing multi-page selection | Owner/Admin | Members list contains more than one page | 1. Select all rows on page 1 2. Move to page 2 3. Select one or more additional rows | Previous page selections remain intact and newly selected rows are added to the same bulk selection set. | High |
| MEM-021E | Clear all selected rows from the current bulk selection | Owner/Admin | At least one member row is selected | 1. Use the bulk-clear path provided by the page 2. Observe selection state | All selected rows are cleared and bulk action state resets cleanly. | Medium |
...
| MEM-027A | Restrict browser autofill suggestions on Add Member name entry | Owner/Admin | Browser has saved text history for general text inputs | 1. Open `/members/new` 2. Focus `Full Name` | Add Member entry does not get cluttered by irrelevant saved-name browser suggestions that block the form visually. | Medium |
...
| MEM-037A | Record payment successfully with payment method | Owner/Admin | Selected member has pending amount greater than zero | 1. Open `Record Payment` 2. Enter valid amount 3. Select `Cash`, `UPI`, or `Card` 4. Confirm | Payment is recorded successfully with the selected payment method preserved in the transaction context. | High |
...
| MEM-045A | Capture payment method during renewal or plan-change collection | Owner/Admin | Renewal or plan-change flow includes a collected-now amount | 1. Open renewal or change-plan flow 2. Enter collected amount 3. Select `Cash`, `UPI`, or `Card` 4. Submit | Membership update succeeds and collected-now payment records the chosen payment method correctly. | High |
...
| MEM-057A | Export selected member records | Owner/Admin | At least one member row is selected | 1. Select one or more members 2. Trigger export action | Export includes the selected records only and completes without breaking page state. | High |
| MEM-057B | Delete selected member records in bulk | Owner/Admin | At least one deletable member row is selected | 1. Select multiple members 2. Trigger bulk delete 3. Confirm action | Selected records are deleted successfully. Remaining list refreshes correctly and selection state resets. | Critical |
